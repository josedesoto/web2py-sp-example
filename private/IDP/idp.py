...
class SSO(Service):
    ...
    def login(self):
        ...
        '''
        logger.info("--- IDP initiated SSO ---")
        client = Saml2Client()
        binding = BINDING_HTTP_POST 
        req_id, req = client.create_authn_request(binding=binding)
        ...
        self.user = "josedesoto@gmail.com"
        #self.environ["idp.authn"] = AUTHN_BROKER.get_authn_by_accr(PASSWORD)
        self.response_bindings = [BINDING_HTTP_POST]
        _dict = {"SAMLRequest": ""}
        return self.operation(_dict, BINDING_HTTP_POST)
